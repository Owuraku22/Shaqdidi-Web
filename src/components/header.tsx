...
export default function Header({ title, showPSLogo = false }: HeaderProps) {
  ...
  const {
    setNotifications: setNotificationsStore,
  ...
  return (
    <header
      className={`bg-white ${
        showPSLogo ? "shadow-sm " : "shadow border-b border-gray-200"
      } px-4 md:px-10 py-2 flex justify-between w-full items-center`}
    >
                ...
                {notifications?.length === 0 ? (
                  ...
                  <div className="divide-y">
                    {notifications?.map((notification) => (
                      <div
                        key={notification.id}
                        className="p-4 relative hover:bg-gray-50"
                      >
                        <button
                          onClick={() =>
        ...
        <Popover>
          ...
          <PopoverContent className="w-[240px] p-4">
            <div className="space-y-2">
              <h5 className="font-semibold">{user?.name || "User"}</h5>
              <p className="text-sm text-gray-500">
                {user?.email || "No email available"}
              </p>
              <p className="text-sm text-gray-500">
                {user?.phone_number || "0239488858"}
              </p>
              {!showPSLogo && (
                <button
